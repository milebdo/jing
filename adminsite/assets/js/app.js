$(function() {
	...
	$(".widget-write-post textarea").limit({
		limit: 20
	});
	...
	$('#prevtab').on('click', function() {
		$('.tabbable li.itemtab').filter('.active').prev('li.itemtab').find('a[data-toggle="tab"]').tab('show');
	});
	$('#nexttab').on('click', function() {
		$('.tabbable li.itemtab').filter('.active').next('li.itemtab').find('a[data-toggle="tab"]').tab('show');
	});
	...
	$('.widget-chat .chat-collapse').on('click', function() {
		var btn = $(this),
			chatBody = btn.closest(".widget-chat").find(".chat-body");
					...
					objBtn.each(function(i) {
						...
						btn.each(function(i) {
							...
							$(this).css({
								...
								"color": group.data("btn-fcolor") || "#FFF"
							});
						});
					});
				}
	...
	$("a.im-confirm").click(function() {
		var aClick = $(this),
			confirm = aClick.data("confirm"),
			parents = aClick.closest('li');
	...
	$('.progress-tooltip .progress-bar').progressbar({
	...
	$('.popover-area-hover [data-toggle="popover"]').popover({
		trigger: "hover"
	});
	...
	$(".form-horizontal").each(function() {
		var form = $(this),
			data = $(this).data(),
			colClass = "col-" + (data.col || "md"),
			coldiv = "",
			colOffset = colClass + "-offset-",
			labelClass = colClass + "-" + (data.collabel || "2");
		if (data.collabel) {
		...
		/*$(this).find(".form-group").each(function(i) {
			...
			if (data.label) {
				...
				$label.find("span.icon").css({
					"border-color": data.colorlabel || "#EEE"
				});
			}
	...
	var createiCheck = (function() {
		$('.iCheck').each(function(i) {
			var data = $(this).data(),
				...
				li = $(this).find("li"),
				index = "cp" + i,
				insert_text, iCheckColor = ["black", "red", "green", "blue", "aero", "grey", "orange", "yellow", "pink", "purple"],
	...
	$('#colorpalette_events').colorPalette({
		colors: [cc_color]
	...
	$(".panel-tools").on('click', ".btn-close", function() {
		var panel = $(this).closest('.panel'),
			tools = $(this).closest('.panel-tools');
		console.log(tools)
			...
			$(this).graphTable({
				...
				series: {
					...
					pie: {
						...
						stroke: {
							shadow: data.pieStyle == "shadow" ? true : false
						},
						label: {
							show: data.pieLabel
						}
				...
				yaxis: {
					...
					tickFormatter: function number(x) {
						...
						return x > 0 ? 'Rp.' + $.number(x,0,',','.') : 0;

						if (x >= 1000000000) {
							num = Math.round(x / 1000000000) + " m";
						} else if (x >= 1000000) {
							num = Math.round(x / 1000000) + " jt";
						} else if (x >= 1000) {
							num = Math.round(x / 1000) + " ribu";
						} else {
							num = x;
						}
						return num;
	...
	$(".label-flot-custom").each(function() {
		...
		if ($(data.flotId).prev(".label-flot-custom-title")) {
			...
			$(data.flotId).prev(".label-flot-custom-title").css({
				"height": height,
				"line-height": height
			});
		}
	...
	$('.easy-chart').each(function() {
		...
		$data.scaleColor = $data.scaleColor || false, $data.onStep = function(from, to, percent) {
			$(this.el).find('.percent').text(Math.round(percent));
		}
		thisEasy.find('.percent').css({
	...
	$('.ajax-load-form').live('click',function(event){
			...
			$.ajax({
				...
				success	 : function (response) {
					if (response.id) {
						...
						form.attr('action',form.data('action') + '/' + response.id);

						$.each(response.data,function(key,value){
							if (form.find('.form-input#' + key).length) {
								form.find('.form-input#' + key).val(value);
							}
							if (form.find('.form-image#' + key).length) {
								var src = form.find('.form-image#' + key).data('src');
								if (value) {
									form.find('.form-image#' + key).attr('src',src + value);
								} else {
									form.find('.form-image#' + key).attr('src','');
								}
							}
						});
