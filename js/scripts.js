function tagging(){
	...
	var base_url2 = l.protocol + "//" + l.host + "/" + l.pathname.split('/')[1] + "/" + "adminsite" + "/";
...
//    $( document ).on( 'click',  'body', function(){
    $("body").click(function(){
     ...
     $( document ).on( 'click',  '[id^="view_"]', function(){
        ...
        $.ajax({
			...
			data: { id: value }
				...
				$("#nokavling").val('No. Kavling : '+jsonparsing.nokav);
				$("#lb").val('Luas Bangunan : '+jsonparsing.lb+' m2');
				$("#lt").val('Luas Bangunan : '+jsonparsing.lt+' m2');
				$("select[name='status']").val(jsonparsing.status);
				$("#imageup").attr("src",base_url2 + jsonparsing.img);
				if(jsonparsing.status == '0') status = "Belum Terjual";
				if(jsonparsing.status == '1') status = "Terjual";
				if(jsonparsing.status == '2') status = "Ready Stock";
				$("#status").val('Status : '+status);		

				if(jsonparsing.status == '1') {
        ...
        $("#"+id).append( '<div id="tagit" style="margin-left:50px;">' +
				'<div class="name"><div class="showup" style="padding-bottom:5px;"><input id="htype" type="text"></div>'+
        		  '<div><img width=250 style="margin-bottom:10px;" src="" id="imageup" /></div></div>'+	
        		  '<div class="desc"><div class="showdesc"><input id="nokavling" type="text"><br />'+
        		  '<input id="lt" type="text"></br />'+
        		  '<input id="lb" type="text"><br />'+
        		  '<input id="status" type="text"></div><input style="margin-top:10px;" type="button" class="button" name="purchase" value="Booking Sekarang" id="purchase" /></div>'+		     	      
	     	      '</div>' );
     ...
     $('#myinputform').ajaxForm(function(data) {
			...
			{			
				$("#popup").fadeOut('slow');
				document.getElementById('message').innerHTML = 'Terima Kasih, Tim Marketing kami akan segera menghubungi anda';
				$("#success").show('slow');
			}
			else
			{
				$("#popup").fadeOut('slow');
				document.getElementById('message').innerHTML = 'Maaf, Data gagal dikirim, silahkan coba lagi';
				$("#success").show('slow');
			}
 });
}
